 ...
 *
 * This single Express app handles every `/api/*` route (see vercel.json,
 * which rewrites all `/api/*` requests here). Vercel invokes this file as a
 * request handler for each request; there is intentionally no `app.listen()`
 * — Vercel's runtime manages the HTTP server for us.
 */
import express from 'express';
import crypto from 'crypto';
import { getDb, ensureTablesExist } from './_lib/db';
...
// 2. Email registration (Neon database)
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!sql) {
    return res.status(503).json({
      error: 'Database Neon belum terhubung. Pastikan DATABASE_URL telah diset pada Environment Variables Vercel.',
    });
  }
...
// 4. Fetch full user financial data (transactions, categories, reminders)
...
// Vercel invokes this exported Express app directly as the request handler
// for every request matched to this function (see the rewrite in vercel.json).
export default app;
